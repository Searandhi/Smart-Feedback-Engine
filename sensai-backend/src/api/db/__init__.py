...
practice_assignments_table_name = "practice_assignments"
assignment_submissions_table_name = "assignment_submissions"
...
async def create_practice_assignments_table(cursor):
    await cursor.execute(
        f"""CREATE TABLE IF NOT EXISTS {practice_assignments_table_name} (
                id INTEGER PRIMARY KEY AUTOINCREMENT,
                course_id INTEGER NOT NULL,
                title TEXT NOT NULL,
    ...
    await cursor.execute(
        f"""CREATE INDEX IF NOT EXISTS idx_practice_assignment_course_id ON {practice_assignments_table_name} (course_id)"""
    )


async def create_assignment_submissions_table(cursor):
    await cursor.execute(
        f"""CREATE TABLE IF NOT EXISTS {assignment_submissions_table_name} (
                id INTEGER PRIMARY KEY AUTOINCREMENT,
                ...
                smart_feedback TEXT DEFAULT NULL,
                submitted_at DATETIME DEFAULT CURRENT_TIMESTAMP,
                FOREIGN KEY (assignment_id) REFERENCES {practice_assignments_table_name}(id) ON DELETE CASCADE
            )"""
    )
    await cursor.execute(
        f"""CREATE INDEX IF NOT EXISTS idx_submission_assignment_id ON {assignment_submissions_table_name} (assignment_id)"""
    )
