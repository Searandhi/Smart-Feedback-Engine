...
async def create_practice_assignments_migration():
    """
    Migration: Creates the practice_assignments table if it doesn't exist.
    Safe to run on existing databases. Also adds language/input_type columns.
    """
    async with get_new_db_connection() as conn:
        cursor = await conn.cursor()
        from api.db import create_practice_assignments_table

        await create_practice_assignments_table(cursor)

        # Alter table for backwards compatibility
        from api.db import practice_assignments_table_name
        
        await cursor.execute(f"PRAGMA table_info({practice_assignments_table_name})")
        existing_columns = [col[1] for col in await cursor.fetchall()]

        if "language" not in existing_columns:
            await cursor.execute(f"ALTER TABLE {practice_assignments_table_name} ADD COLUMN language TEXT NOT NULL DEFAULT 'python'")
            await cursor.execute(f"ALTER TABLE {practice_assignments_table_name} ADD COLUMN input_type TEXT NOT NULL DEFAULT 'code'")

        await conn.commit()


async def create_assignment_submissions_migration():
    """
    Migration: Creates the assignment_submissions table if it doesn't exist.
    Safe to run on existing databases. Also adds smart_feedback column.
    """
    async with get_new_db_connection() as conn:
        cursor = await conn.cursor()
        from api.db import create_assignment_submissions_table

        await create_assignment_submissions_table(cursor)

        from api.db import assignment_submissions_table_name
        
        await cursor.execute(f"PRAGMA table_info({assignment_submissions_table_name})")
        existing_columns = [col[1] for col in await cursor.fetchall()]

        if "smart_feedback" not in existing_columns:
            await cursor.execute(f"ALTER TABLE {assignment_submissions_table_name} ADD COLUMN smart_feedback TEXT DEFAULT NULL")
...
async def run_migrations():
    ...
    await create_practice_assignments_migration()
    await create_assignment_submissions_migration()
