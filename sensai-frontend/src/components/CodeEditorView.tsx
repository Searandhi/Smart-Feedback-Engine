...
const CodeEditorView = forwardRef<CodeEditorViewHandle, CodeEditorViewProps>(({
    ...
    useEffect(() => {
        const nextState = setupCodeState(initialCode);
        // Only update if it's actually different from current state to prevent loops
        if (JSON.stringify(nextState) !== JSON.stringify(code)) {
    ...
    // Use an effect to notify the parent of code changes.
    // This ensures the update happens after the render cycle, 
    // avoiding the "Cannot update a component while rendering a different component" error.
    useEffect(() => {
        // Only notify if it was an internal change (from user interaction)
        if (onCodeChange && isInternalChange.current) {
            onCodeChange(code);
    ...
    const executeWithJudge0 = async (language: string, sourceCode: string) => {
        try {
            ...
            if (!createResponse.ok) {
                ...
                throw new Error(errorData.error || `Failed to submit code: ${createResponse.status}`);
            }
