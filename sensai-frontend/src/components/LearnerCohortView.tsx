        ...
        <div className="bg-white dark:bg-black min-h-screen pb-16 lg:pb-0" role="main">
            ...
            <div className="lg:flex lg:flex-row lg:justify-between">
                ...
                <div className={`lg:w-2/3 lg:pr-8 ${showSidebar && activeMobileTab === MobileTab.Progress ? 'hidden lg:block' : ''}`}>
                    {/* Course Selector + Assignments Tab - always visible */}
                        ...
                        <div className="hidden sm:block w-full">
                            <div className="flex items-center border-b overflow-x-auto scrollbar-hide border-gray-300 dark:border-gray-900">
                                {courses.map((course, index) => (
                                    <button
                                        key={course.id}
                                        className={`px-8 py-4 text-base md:text-lg tracking-wide whitespace-nowrap transition-all duration-200 cursor-pointer flex-shrink-0 relative group ${index === activeCourseIndex && activeMobileTab !== MobileTab.Assignments
                                            ? 'text-black dark:text-white font-light'
                                            : 'text-gray-500 hover:text-gray-700 dark:hover:text-gray-300 font-light'
                                            }`}
                                        onClick={() => { handleCourseSelect(index); setActiveMobileTab(MobileTab.Course); }}
                                        ref={el => { courseTabRefs.current[index] = el; }}
                                    >
                                        <span className="relative z-10">{course.name}</span>
                                        {index === activeCourseIndex && activeMobileTab !== MobileTab.Assignments && (
                                ...
                                {/* Assignments tab - always visible */}
                                <button
                                    id="assignments-tab-btn"
                                    className={`px-8 py-4 text-base md:text-lg tracking-wide whitespace-nowrap transition-all duration-200 cursor-pointer flex-shrink-0 relative group flex items-center gap-2 ${
                                        activeMobileTab === MobileTab.Assignments
                                            ? 'text-indigo-600 dark:text-indigo-400 font-light'
                                            : 'text-gray-500 hover:text-gray-700 dark:hover:text-gray-300 font-light'
                                    }`}
                                    onClick={() => setActiveMobileTab(MobileTab.Assignments)}
                                >
                                    <ClipboardList className="w-4 h-4" />
                                    <span className="relative z-10">Assignments</span>
                                    {activeMobileTab === MobileTab.Assignments && (
                                        <div className="absolute bottom-0 left-0 right-0 h-[1px] bg-indigo-600 dark:bg-indigo-400" />
                                    )}
                                    {activeMobileTab !== MobileTab.Assignments && (
                                        <div className="absolute bottom-0 left-0 right-0 h-[1px] scale-x-0 group-hover:scale-x-100 transition-transform duration-200 origin-left bg-gray-300 dark:bg-gray-700" />
                                    )}
            ...
            {showSidebar && (
                <div className="lg:hidden fixed bottom-0 left-0 right-0 border-t z-20 bg-gradient-to-t from-white to-[rgba(255,255,255,0.9)] dark:from-black dark:to-[rgba(0,0,0,0.9)] border-gray-300 dark:border-gray-900">
                    <div className="flex h-16">
                        ...
                        <button
                            className={`flex-1 flex flex-col items-center justify-center transition-colors ${activeMobileTab === MobileTab.Assignments
                                ? 'text-indigo-600 dark:text-indigo-400'
                                : 'text-gray-500'
                                }`}
                            onClick={() => setActiveMobileTab(MobileTab.Assignments)}
                            id="mobile-assignments-tab"
                        >
